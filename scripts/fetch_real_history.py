...
print(f"🌍 Téléchargement Historique (Cibles 24h) : {START_DATE} -> {END_DATE}...")
...
for city in CITIES:
    ...
    try:
        ...
        if 'hourly' not in resp: continue

        hourly = resp['hourly']
        count = len(hourly['time'])
        
        for i in range(count - 48):
            current_hour = int(hourly['time'][i].split('T')[1].split(':')[0])
            is_day = 1 if 6 <= current_hour <= 20 else 0
            ...
            # Calcul du décalage pour atteindre minuit demain
            hours_to_midnight = 24 - current_hour
            
            valid = True
            for h in TARGET_HOURS:
                # On vise l'heure H de demain
                target_idx = i + hours_to_midnight + h
                if target_idx < count:
                    row[f"T_{h:02d}H"] = hourly['temperature_2m'][target_idx]
                else:
                    valid = False; break
            
            if valid: all_data.append(row)
                
    except Exception as e: print(f"❌ Erreur {city['name']}: {e}")
    time.sleep(0.1)

df = pd.DataFrame(all_data)
df = df.dropna()
df.to_csv("weather_history_real.csv", index=False)
print(f"✅ Historique prêt : {len(df)} lignes.")